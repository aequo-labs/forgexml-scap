...
// It recursively searches the entire document tree for elements matching the type name.
func (s *XMLDocumentState) ListElementsByType(typeName string) ([]interface{}, error) {
	...
	s.findElementsByType(s.rootElement, typeName, &results)
	return results, nil
}

// findElementsByType recursively searches for elements matching the given type name.
func (s *XMLDocumentState) findElementsByType(element interface{}, typeName string, results *[]interface{}) {
	if element == nil {
		return
	}
	...
	// Check if this element matches the type name
	elemType := val.Type().Name()
	...
	for i := 0; i < val.NumField(); i++ {
		...
		// Skip unexported fields and XMLName
		...
		if xmlTag != "" && (strings.Contains(xmlTag, ",attr") || xmlTag == "-") {
			continue
		}
		...
		if field.Kind() == reflect.Slice {
			for j := 0; j < field.Len(); j++ {
				...
				if item.Kind() == reflect.Ptr || item.Kind() == reflect.Struct {
					s.findElementsByType(item.Interface(), typeName, results)
				}
			}
		} else if field.Kind() == reflect.Ptr && !field.IsNil() {
			s.findElementsByType(field.Interface(), typeName, results)
		} else if field.Kind() == reflect.Struct {
			s.findElementsByType(field.Addr().Interface(), typeName, results)
		}
	}
}
