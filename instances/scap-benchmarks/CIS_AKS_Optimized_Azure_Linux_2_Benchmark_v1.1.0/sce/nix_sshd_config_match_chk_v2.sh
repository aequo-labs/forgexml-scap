...
# E. Pinnell   02/20/25   Modified to correctly match users that are a member of the matched group (Bug fix)
   ...
   for l_sshd_config_file in "${a_sshd_config_files[@]}"; do
      while IFS=" " read -r _ l_match l_match_value; do
         ...
         case "${l_match,,}" in
            ...
            group )
               l_match_type="user"
               l_match_value="$(getent group "$l_match_value" | awk -F':' '{split($4, a, ","); print a[1]}')"
               f_match_value_chk ;;
            host )
               l_match_type="host"
               f_match_value_chk ;;
