...
if [ -n "$(echo "$REGEX" | grep -E 'auid(>|>=|=>)')" ] ; then
	sysuid="$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)"
	REGEXCHK="$(echo "$REGEX" | sed -r "s/^(.*)(auid(>=|>\>=).*)(\s+-[A-Z].*)$/\1auid\3$sysuid\4/")"
	output="$(grep -E -s "$REGEXCHK" /etc/audit/rules.d/*.rules | cut -d: -f2)"
	location="$(grep -E -s "$REGEXCHK" /etc/audit/rules.d/*.rules | cut -d: -f1)"
else
	output="$(grep -E -s "$REGEX" /etc/audit/rules.d/*.rules | cut -d: -f2)"
	location="$(grep -E -s "$REGEX" /etc/audit/rules.d/*.rules | cut -d: -f1)"
fi
