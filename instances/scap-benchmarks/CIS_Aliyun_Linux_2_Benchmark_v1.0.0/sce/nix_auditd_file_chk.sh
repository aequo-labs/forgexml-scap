...
#	REGEXCHK="$(echo "$REGEX" | sed -r "s/auid(>=|>|=>)([0-9]+)/auid>=$sysuid/g")"
	REGEXCHK="$(printf '%s' "$REGEX" | sed -r "s/auid(>=|>|=>)([0-9]+)/auid>=$sysuid/g")"
	...
	location="$(grep -P -l -s -- "$REGEX" /etc/audit/rules.d/*.rules)"
fi
