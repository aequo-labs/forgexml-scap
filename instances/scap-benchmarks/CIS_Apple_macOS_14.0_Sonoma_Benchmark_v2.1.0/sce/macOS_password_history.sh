...
# Edward Byrd		  06/18/25	 Update for new audit
# 

pwhistory=$(
pref1=$(/usr/bin/pwpolicy -getaccountpolicies | /usr/bin/grep -A1 policyAttributePasswordHistoryDepth | /usr/bin/tail -1 | /usr/bin/cut -d'>' -f2 | /usr/bin/cut -d '<' -f1) && pref2=$(/usr/bin/pwpolicy -getaccountpolicies | /usr/bin/grep -A1 policyAttributePasswordHistoryDepth | /usr/bin/tail -1 | /usr/bin/cut -d'>' -f2 | /usr/bin/cut -d '<' -f1) && if [[ "$pref1" != "" && pref1 -ge 15 ]]; then echo "true" && pwhistoryvalue=$pref1; elif [[ "$pref2" != "" && pref2 -ge 24 ]]; then echo "true" && pwhistoryvalue=$pref2; else echo "false"; fi
)
