...
# Edward Byrd		  06/18/25	 Update to output for new formatting implemented by Apple
#

pwminlength=$(
/usr/bin/pwpolicy -getaccountpolicies | /usr/bin/grep -e "policyAttributePassword matches" | /usr/bin/grep -e "[0-9][0-9]" | /usr/bin/cut -b 46-53 | /usr/bin/cut -d',' -f1 | /usr/bin/cut -d'{' -f2
)

if [ "$pwminlength" != "" ] && [ $pwminlength -ge 15 ]; then
  output=True
else
  output=False
fi
