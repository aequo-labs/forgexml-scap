...
# E. Pinnell     11/22/24   Modified to work on systems without the unified filesystem
#
# Note: 
...
# XCCDF_VALUE_REGEX="ForwardToSyslog=yes:systemd/journald.conf" # <- XCCDF_VALUE_REGEX example
# XCCDF_VALUE_REGEX="^\h*([^#\n\r]+)?\h+\/[^#\n\r]+\d{3,}=^.*$:/etc/tmpfiles.d/systemd.conf"

a_output=(); a_output2=()
l_analyze_cmd="$(readlink -f /bin/systemd-analyze || readlink -f /usr/bin/systemd-analyze)"
