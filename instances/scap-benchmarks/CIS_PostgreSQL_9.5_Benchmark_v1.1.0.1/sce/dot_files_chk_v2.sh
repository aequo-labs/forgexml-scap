...
for dir in $(awk -F: '($1 !~ /^(root|halt|sync|shutdown)$/ && $7 != "'"$(which nologin)"'" && $7 != "/bin/false" && $7 != "/usr/bin/false") { print $6 }' /etc/passwd); do
	if [ -e "$dir"/"$REGEX" ]; then
		[ -z "$output" ] && output="Failed, file(s) exist: \"$dir/$REGEX\"" || output="$output, \"$dir/$REGEX\""
	fi
done
...
if [ "$passing" = true ] ; then
	echo "Passed, no \"$REGEX\" files exist"
...
if [ "$passing" = true ] ; then
	echo "Passed: No \"$REGEX\" file exist in users' home directories"
