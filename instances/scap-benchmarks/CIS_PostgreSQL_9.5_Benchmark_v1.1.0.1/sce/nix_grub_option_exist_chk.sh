...
if [ -f "$grubfile" ]; then
	if [ "$gfsn" = "grub.conf" ]; then
		! grep -P "^\s*kernel" "$grubfile" | grep -Pvq "\b$REGEX\b" && passing=true
		output="$(grep -P '^\h*kernel.*$' "$grubfile" | grep -P "\b$REGEX\b")"
	elif [ "$gfsn" = "grub.cfg" ]; then
		! grep -P "^\h*linux.*$" "$grubfile" | grep -Pvq "\b$REGEX\b" && passing=true
		output="$(grep -P "^\h*linux.*$" "$grubfile" | grep -P "\b$REGEX\b")"
	elif [ "$gfsn" = "grubenv" ]; then
		! grep -P "^\h*kernelopts=.*$" "$grubfile" | grep -Pvq "\b$REGEX\b" && passing=true
