...
if [ -f "$grubfile" ]; then
	if [ "$gfsn" = "grub.conf" ]; then
		! grep -P "^\s*kernel" "$grubfile" | grep -Pq "\b$REGEX\b" && passing=true
		output="$(grep -P '^\h*kernel.*$' "$grubfile" | grep -P "\b$REGEX\b")"
	elif [ "$gfsn" = "grub.cfg" ]; then
		! grep -P "^\h*linux.*$" "$grubfile" | grep -Pq "\b$REGEX\b" && passing=true
		output="$(grep -P "^\h*linux.*$" "$grubfile" | grep -P "\b$REGEX\b")"
	elif [ "$gfsn" = "grubenv" ]; then
		! grep -P "^\h*kernelopts=" "$grubfile" | grep -Pq "\b$REGEX\b" && passing=true
		output="$(grep -P "^\h*kernelopts=.*$" "$grubfile" | grep -P "\b$REGEX\b")"
	fi
fi
...
if [ "$passing" = true ] ; then
	echo "PASSED"
	echo "\"$grubfile\" doesn't contain: \"$REGEX\""
	exit "${XCCDF_RESULT_PASS:-101}"
else
	# print the reason why we are failing
	echo "FAILED:"
	echo "\"$grubfile\" contains: \"$REGEX\""
	echo $output
