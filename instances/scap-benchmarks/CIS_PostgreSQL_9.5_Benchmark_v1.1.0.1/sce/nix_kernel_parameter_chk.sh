...
# E. Pinnell   12/03/21   Modified to deal with potential white space in the REGEX variable

tst1="" tst2="" tst3="" passing=""

kpname=$(printf "%s" "$REGEX" | awk -F= '{print $1}' | xargs)
kpvalue=$(printf "%s" "$REGEX" | awk -F= '{print $2}' | xargs)
