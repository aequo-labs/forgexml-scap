...
if command -v lsblk >/dev/null; then
   for rmpo in $(lsblk -o RM,MOUNTPOINT | awk -F " " '/1/ {print $2}'); do
      if ! findmnt -n "$rmpo" | grep -Evq "\b$REGEX\b"; then
...
if [ "$passing" = true ] ; then
   [ -n "$output" ] && echo "Manual Check Required" || echo "PASSED! Removable media partition(s) include the $REGEX option"
   exit "${XCCDF_RESULT_PASS:-101}"
else
   # print the reason why we are failing
   echo "FAILED!"
   [ -n "$output" ] && echo "$output"
   echo "Don't include the $REGEX option"
