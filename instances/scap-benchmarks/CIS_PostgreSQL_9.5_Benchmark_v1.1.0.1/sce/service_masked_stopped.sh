...
tst1="$(systemctl show "$REGEX" | awk -F '=' '/UnitFileState=/ {print $2}')"
[ -z "$tst1" ] && tst1="$(systemctl show "$REGEX" | awk -F '=' '/LoadState=/ {print $2}')"
tst2="$(systemctl show "$REGEX" | awk -F '=' '/ActiveState=/ {print $2}')"
output="Service $REGEX is $tst1 and $tst2"
