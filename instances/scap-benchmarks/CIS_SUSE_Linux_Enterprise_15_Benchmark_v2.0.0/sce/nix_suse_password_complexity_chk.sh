...
# -------------------------------------------------------------------
# R.Bejar      12/03/24  suse password complexity check
...
{
   for l_pam_file in /etc/pam.d/common-password /etc/pam.d/common-password-pc; do
      if grep -Psiq -- '^\h*password\h+(requisite|required|sufficient)\h+pam_cracklib\.so\h+([^#\n\r]+\h+)?'"$l_prc"'\b' "$l_pam_file"; then
...
for l_credit_name in "${a_credits[@]}"; do
   if ! grep -Psiq '^\h*'"$l_credit_name"'\b' /etc/security/pwquality.conf /etc/security/pwquality.conf.d/*.conf; then
      if ! grep -Psiq -- '^\h*password\h+(requisite|required|sufficient)\h+pam_cracklib\.so\h+([^#\n\r]+\h+)?'"$l_credit_name"'\b' /etc/pam.d/common-password /etc/pam.d/common-password-pc; then
