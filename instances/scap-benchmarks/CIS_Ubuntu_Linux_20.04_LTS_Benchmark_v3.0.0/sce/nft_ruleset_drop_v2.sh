...
# R. Bejar     02/19/25   updated to nft -y 

# supersedes nft_ruleset_drop.sh

CHAIN="$XCCDF_VALUE_REGEX"

nft list -y ruleset | grep -Eq "^\s*type\s+filter\s+hook\s+$CHAIN\s+(priority\s+\S+;\s+)*policy\s+drop;" && passing=true
