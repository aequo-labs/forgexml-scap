...
# R. Bejar       02/25/25   Updated to include UDP checks and updated TCP obsolete greps

# supersedes nix_rsyslog_receive_logs_chk.sh

# Initalize arrays and set variables
a_output2=() a_config_files=("/etc/rsyslog.conf" "/etc/rsyslog.d/"*.conf)
...
for l_logfile in "${a_config_files[@]}"; do
   ...
   [ -n "$l_fail" ] && a_output2+=("- Advanced format entry to accept incoming logs: \"$l_fail\"" "found in: \"$l_logfile\"")
   l_fail="$(grep -Psi -- '^\h*(\$ModLoad imtcp)' "$l_logfile")"
   [ -n "$l_fail" ] && a_output2+=("- Obsolete format entry to accept incoming logs: \"$l_fail\"" "found in: \"$l_logfile\"")
   l_fail="$(grep -Psi -- '^\h*(\$InputTCPServerRun)' "$l_logfile")"
   [ -n "$l_fail" ] && a_output2+=("- Obsolete format entry to accept incoming logs: \"$l_fail\"" "found in: \"$l_logfile\"")
done

# check for UDP configuration
if grep -P '^\s*(module\(load="imudp"\)|input\(type="imudp" port="514"\))' "${a_config_files[@]}"; then 
   a_output2+=("UDP is configured using Advanced format entry to accept incoming logs found in: \"$l_logfile\"")
else
   if grep -P '^\s*(\$ModLoad imudp)|^\$UDPServerRun' "${a_config_files[@]}"; then   
      a_output2+=("UDP is configured using obsolete legacy format entry to accept incoming logs found in: \"$l_logfile\"")
   fi   
fi  
