...
# E. Pinnell 04/28/25   Modified to ignore all of /run and "*/kubelet/*"
...
a_path=(! -path "/run/*" -a ! -path "/proc/*" -a ! -path "*/containerd/*" -a ! -path "*/kubelet/*" -a ! -path "/sys/fs/cgroup/memory/*" -a ! -path "/var/*/private/*")
...
done < <(findmnt -Dkerno fstype,target | awk '($1 !~ /^\s*(nfs|proc|cifs|smb|vfat|iso9660|efivarfs|selinuxfs)/ && $2 !~ /^\/run\/user\//){print $2}')
