...
//   github.com/aequo-labs/forgexml/pkg/generator/templates/validator.go.tmpl
//
// Then regenerate using forgexml and fix import paths.

// This file provides XML validation utilities including struct validation,
// element counting, and round-trip fidelity comparison.
...
func ValidateStruct(data any) (*ValidationResult, error) {
...
// extractLocalName extracts the Local field from an XMLName or Name struct field.
func extractLocalName(structVal reflect.Value, nameField string) string {
	field := structVal.FieldByName(nameField)
	if !field.IsValid() || field.Kind() != reflect.Struct {
		return ""
	}
	localField := field.FieldByName("Local")
	if !localField.IsValid() || localField.Kind() != reflect.String {
		return ""
	}
	return localField.String()
}

// collectUnknownElements extracts element names from UnknownElements field.
func collectUnknownElements(field reflect.Value, result *ValidationResult) {
	result.UnknownElements = field.Len()
	for i := 0; i < field.Len(); i++ {
		elem := field.Index(i)
		if elem.Kind() == reflect.Struct {
			if name := extractLocalName(elem, "XMLName"); name != "" {
				result.UnknownElementsList = append(result.UnknownElementsList, name)
			}
		}
	}
}

// collectUnknownAttrs extracts attribute names from UnknownAttrs field.
func collectUnknownAttrs(field reflect.Value, result *ValidationResult) {
	result.UnknownAttrs = field.Len()
	for i := 0; i < field.Len(); i++ {
		attr := field.Index(i)
		if attr.Kind() == reflect.Struct {
			if name := extractLocalName(attr, "Name"); name != "" {
				result.UnknownAttrsList = append(result.UnknownAttrsList, name)
			}
		}
	}
}

// validateNestedValue recursively validates a nested struct or pointer to struct.
func validateNestedValue(val reflect.Value, path string, result *ValidationResult) {
	switch val.Kind() {
	case reflect.Struct:
		validateStructFields(val, path, result)
	case reflect.Ptr:
		if !val.IsNil() && val.Elem().Kind() == reflect.Struct {
			validateStructFields(val.Elem(), path, result)
		}
	}
}

// validateSliceElements validates each element in a slice field.
func validateSliceElements(field reflect.Value, basePath string, result *ValidationResult) {
	for j := 0; j < field.Len(); j++ {
		elem := field.Index(j)
		elemPath := fmt.Sprintf("%s[%d]", basePath, j)
		validateNestedValue(elem, elemPath, result)
	}
}
...
func validateStructFields(val reflect.Value, prefix string, result *ValidationResult) {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case "UnknownElements":
			collectUnknownElements(field, result)
			continue
		case "UnknownAttrs":
			collectUnknownAttrs(field, result)
			continue
		}
		...
		case reflect.Struct, reflect.Ptr:
			validateNestedValue(field, fullName, result)
		case reflect.Slice:
			if field.Len() > 0 {
				validateSliceElements(field, fullName, result)
			}
		}
	}
}
...
// CountMatch holds element or attribute count comparison results.
...
// NameMatch holds element or attribute name matching results.
...
// NameChange records an element name that changed between input and output.
...
// ValueMatch holds element or attribute value comparison results.
...
// ValueMismatch records a value that differs between input and output XML.
...
// DepthMatch holds XML tree depth comparison results.
...
// ParseXMLForComparison parses XML into a comparable tree structure.
...
// XMLNode represents a node in the parsed XML tree for comparison.
type XMLNode struct {
	Name       xml.Name
	Attributes []xml.Attr
	Value      string
...
// buildPath constructs the XPath-like path for an element.
